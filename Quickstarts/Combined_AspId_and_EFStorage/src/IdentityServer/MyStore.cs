...
{
    public class MyStore : IPersistedGrantStore
    {
        ...
        public MyStore(IPersistedGrantDbContext context, ILogger<MyStore> logger)
        {
            _context = context;
            _logger = logger;
        }
        ...
        public Task StoreAsync(PersistedGrant token)
        {
            ...
            if (existing == null)
            {
                _logger.LogDebug("MyStore: {persistedGrantKey} not found in database", token.Key);

                var persistedGrant = token.ToEntity();
                _logger.LogDebug("MyStore: entity is created");               
                
                _context.PersistedGrants.Add(persistedGrant);
                _logger.LogDebug("MyStore: {persistedGrantKey} is added to context", token.Key);
            }
            else
            {
...
//            }
        ...
        public Task RemoveAsync(string key)
        {
            ...
            {
                _logger.LogDebug("test: removing {persistedGrantKey} persisted grant from database", key);
